...
local Fz = {
		...
		["Logo Image"] = "rbxassetid://80683120703477",
...
gz.Image = "rbxassetid://80683120703477";
...
local Uz = function(Y)
		...
		local g = D .. ("," .. (A .. ("," .. u)));
		K.Text = "<font color=\"rgb(" .. (g .. (")\">Not Banana</font> " .. dz.TitleNameNoti));
		...
		I.Image = "rbxassetid://80683120703477";
...
function Kz.CreateMain(Y)
	...
	local v = i .. ("," .. (U .. ("," .. C)));
	M.Text = "<font color=\"rgb(" .. (v .. ")\">Not Banana</font> : Blox Fruits");
	...
	local S = b .. ("," .. (c .. ("," .. H)));
	M.Text = "<font color=\"rgb(" .. (S .. ")\">Not Banana</font> : Blox Fruits");
	K.Name = "Background1";
	K.Parent = g;
				...
				elseif Y and (not (CheckShark() and _G.Shark or CheckTerrorShark() and _G.TerrorShark or CheckFishCrew() and _G.MobCrew or CheckPiranha() and _G.Piranha) and (not (CheckEnemiesBoat() and _G.FishBoat) and (not (CheckSeaBeast() and _G.SeaBeast1) and (not (_G.PGB and CheckPirateGrandBrigade()) and (not (_G.HCM and CheckHauntedCrew()) and not (_G.Leviathan1 and CheckLeviathan())))))) then
					if d.Character.Humanoid.Sit == false then
						...
						elseif _G.DangerSc == "Lv Infinite" then
							CFrameSelectedZone = CFrame.new(-10000000, 70, 37016.25);
						end;
						repeat
							wait();
							if not _G.FishBoat and CheckEnemiesBoat() or not _G.PGB and CheckPirateGrandBrigade() or not _G.TerrorShark and CheckTerrorShark() then
								_tp(CFrameSelectedZone * CFrame.new(0, 55, 0));
							else
...
-- ============================================================
-- DUNGEON TAB (added from OSSwithDungeon)
-- ============================================================
local DungeonTab = Cz.T({ Page_Name = "Dungeon", Page_Title = "Dungeon" })
local DungeonSec = DungeonTab:CreateSection("Dungeon Settings")

DungeonSec:CreateToggle({
    Title = "Auto Attack Dungeon Mon",
    Description = "Auto attack nearest dungeon mobs",
    Default = false,
    Callback = function(Y)
        _G.AutoAttackMonDungeon = Y
    end
})
...
DungeonSec:CreateSlider({
    Title = "FastAttack Detection Range",
    Min = 10,
    Max = 500,
    Default = 65,
    Callback = function(Y)
        _G.FastAttackDistance = Y
        if Config then Config.AttackDistance = Y end
    end
})

DungeonSec:CreateButton({
    Title = "Teleport Dungeon Hub",
    Callback = function()
        pcall(function()
...
-- ============================================================
-- BACKEND: FastAttack + Dungeon System (adapted)
-- ============================================================

-- Define missing _G flags
_G.AutoAttackMonDungeon = _G.AutoAttackMonDungeon or false
_G.AutoNextFloor = _G.AutoNextFloor or false
_G.AutoChooseCard = _G.AutoChooseCard or false
_G.AutoReturnToHub = _G.AutoReturnToHub or false
_G.SpinPosition = _G.SpinPosition or false
_G.SpinRadius = _G.SpinRadius or 20
_G.FarmDistance = _G.FarmDistance or 35
_G.FastAttackDistance = _G.FastAttackDistance or 65

-- ================================
-- FASTATTACK MODULE
-- ================================
local RunService = W  -- W is already RunService in notbanana.lua

local function GetBladeHits()
    local t = {}
    local function d(v)
        return (v.Position - d.Character.HumanoidRootPart.Position).Magnitude
    end
...
local Config = {
    AttackDistance = _G.FastAttackDistance or 65,
    AttackMobs = true,
    AttackPlayers = true,
    AttackCooldown = 0.01,
    ComboResetTime = 0.3,
...
function FastAttack.new()
    ...
    pcall(function()
        s.CombatFlags = require(Q.Modules.Flags).COMBAT_REMOTE_THREAD
        local ls = d:FindFirstChild("PlayerScripts"):FindFirstChildOfClass("LocalScript")
...
function FastAttack:_PrimeGun(tl)
    if not tl or not tl.Parent or tl.Parent ~= d.Character then return end
    local now = os.clock()
...
function FastAttack:ShootInTarget(tp)
    ...
    local h = m and (m:FindFirstChild("HumanoidRootPart") or m:FindFirstChild("Head"))
    if not h then return end
    pcall(function()
        Q.Modules.Net["RE/ShootGunEvent"]:FireServer(Vector3.new(tp.X, tp.Y, tp.Z), {h})
    end)
end
...
function FastAttack:UseNormalClick(c, h, cd)
    ...
    if self.EnemyRootPart then
        Q.Modules.Net["RE/RegisterAttack"]:FireServer(cd)
        if self.CombatFlags and self.HitFunction then
            self.HitFunction(self.EnemyRootPart, bh)
        else
            Q.Modules.Net["RE/RegisterHit"]:FireServer(self.EnemyRootPart, bh)
        end
    end
end
...
function ResolveDungeonWeapon()
    ...
    local bp = d:FindFirstChild("Backpack")
    if bp then
    ...
    local charTool = d.Character and d.Character:FindFirstChildOfClass("Tool")
...
function IsDungeonMobAlive(mob, root, highestFloor)
    if not f.Alive(mob) or not mob:FindFirstChild("HumanoidRootPart") or string.find(mob.Name, "Blank Buddy") then
        return false
    end
...
function AttackDungeonMob(mob)
    ...
    f.Kill(mob, _G.AutoAttackMonDungeon)
end
...
task.spawn(function()
    while task.wait(2) do
        if _G.AutoReturnToHub then
            pcall(function()
                local dungeonShared = Q:FindFirstChild("DungeonShared")
...
task.spawn(function()
    while task.wait(0.3) do
        if _G.AutoChooseCard then
            pcall(function()
                for _, gui in pairs(d.PlayerGui:GetChildren()) do
...
-- ================================
-- DUNGEON WEAPON DROPDOWN (adds to Config tab if needed, but placed here for completeness)
-- ================================
-- We already added the UI tab above.
